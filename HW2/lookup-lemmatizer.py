...
for line in train_data:
    ...
    if re.search ('\t' , line):
        ...
        if form in lemma_count:
            if lemma in lemma_count[form].keys():
...
for form in lemma_count.keys():
        ...
        lemma_max[form] = lemma_count[form].keys()
        ...
        for lemma in lemma_count[form].keys():
            if lemma == form:
                training_counts['Identity tokens'] += lemma_count[form][lemma]
...
accuracies['Expected lookup'] = (training_counts['Unambiguous tokens'] + training_counts['Ambiguous most common tokens']) \
...
for line in test_data:
    ...
    if re.search ('\t' , line):
        ...
        if form in lemma_max:
            ...
            correct_lemma = max(lemma_list , key=lambda key: lemma_list[key])
